...
3. 생성된 `orderId`로 `POST /api/payments/prepare`를 호출하고 결제 시도를 식별하는 `paymentId`를 받습니다.
4. PG 승인 후 `paymentId`와 `orderId`로 `POST /api/payments/confirm`을 호출합니다.
...
sequenceDiagram
    ...
    Payment-->>Client: paymentId, orderId, amount
    Client->>Payment: POST /api/payments/confirm(paymentId, orderId)
...
- **Payment**: 결제 시도 1회를 나타내며 상태를 되돌리지 않습니다. 실패 후 재시도는 새로운 Payment로 생성하고, 승인 요청은 준비 단계에서 반환한 `paymentId`로 대상을 식별합니다. 승인된 결제와 승인 후 취소·환불된 결제가 있는 주문은 다시 결제할 수 없습니다.
...
- 최초 결제 준비 요청이 동시에 실행될 때 여러 Payment가 생성되는 경쟁 상태는 아직 제어하지 않습니다. 후속 작업에서 멱등 키 또는 주문 단위 잠금을 검토합니다.
